...
export default async function ClinicDetailPage({
  ...
  const principleBlocks = parseBlocks(clinic.principleBlocks);
  const processSteps = parseBlocks(clinic.processSteps);
  const compareLeft = parseStringList(clinic.compareLeft);
  const compareRight = parseStringList(clinic.compareRight);
  const faq = parseFaq(clinic.faq);
    ...
    <>
      <section className="relative flex min-h-[42vh] items-end overflow-hidden bg-brand-800">
        ...
        <div className="relative mx-auto w-full max-w-4xl px-6 pb-14 pt-24">
          <h1 className="text-4xl font-medium text-ivory-50">{clinic.name}</h1>
      ...
      <div className="mx-auto max-w-4xl px-6 py-16 space-y-20">
        {principleBlocks.length > 0 && (
          <section>
            <h2 className="text-2xl font-medium text-brand-900">Approach</h2>
        ...
        {processSteps.length > 0 && (
          <section>
            <h2 className="text-2xl font-medium text-brand-900">Care Process</h2>
          ...
          <section>
            ...
            <div className="mt-6 grid gap-6 sm:grid-cols-2">
              <div className="rounded-2xl border border-brand-200 bg-brand-50 p-6">
                <h3 className="font-medium text-brand-900">{clinic.compareLeftTitle}</h3>
              ...
              <div className="rounded-2xl border border-brand-100 bg-ivory-50 p-6">
                <h3 className="font-medium text-brand-900">{clinic.compareRightTitle}</h3>
                <ul className="mt-4 space-y-2 text-sm leading-relaxed text-brand-700">
        ...
        <section className="rounded-2xl bg-brand-800 p-10 text-center text-ivory-50">
          <h2 className="text-2xl font-medium">Have questions about this clinic?</h2>
